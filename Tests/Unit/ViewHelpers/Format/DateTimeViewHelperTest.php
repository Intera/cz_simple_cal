...
class DateTimeViewHelperTest extends ViewHelperBaseTestcase
{
    ...
    public function testFormatParameter()
    {
        self::assertEquals(
            '2009-02-13 23:31:30',
            $this->viewHelper->render(1234567890, '%Y-%m-%d %H:%M:%S'),
            'format can be set.'
        );
    ...
    public function testTimestampParameter()
    {
        ...
        self::assertEquals(
            '2009-02-13',
            $this->viewHelper->render('1234567890'),
            'if string only contains numbers it is used as timestamp.'
        );

        self::assertEquals(
            '2009-02-13',
            $this->viewHelper->render('2009-02-13 23:31:30GMT'),
            'if string does not only consist of numbers strtotime is used.'
        );

        self::assertEquals(
            '2009-02-13',
            $this->viewHelper->render(new \DateTime('2009-02-13 23:31:30GMT')),
