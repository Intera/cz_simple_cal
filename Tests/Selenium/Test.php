...
class Test extends BaseSeleniumTestCase
{
    public function testActionList()
    {
        ...
        $this->assertGreaterThan(
            strtotime('2010-01-01 00:00:00UTC'),
            $this->getDateOfFirstEvent(),
            'default for startDate'
        );
        // Default for endDate
        ...
        $this->assertEquals(
            2,
            $this->selenium->getXpathCount('//div[contains(@class, "vevent")]'),
            'testing setting maxEvents'
        );
        ...
        $this->assertSame(
            'overlong event',
            $this->selenium->getText('//div[@class="vevent"][last()]//h5'),
            'testing setting orderBy'
        );
        ...
        $this->assertSame(
            'Wed, January 6, 2010',
            $this->selenium->getText('//div[contains(@class, "vcalendar-list")]//h4'),
        ...
        $this->assertEquals(
            2,
            $this->selenium->getXpathCount('//*[contains(@class, "vevent")]'),
            'testing setting filterCategories'
        );
        ...
        $this->assertSame(
            'Fri, July 9, 2010',
            $this->selenium->getText('//div[contains(@class, "vcalendar-list")]//h4'),
            'testing setting getDate (start at correct date)'
        );
        ...
        $this->assertGreaterThanOrEqual(
            $minDate,
            $firstDate,
            'if getPostAllowed is empty, no config is overridden (#1)'
        );
        ...
        // Override endDate
        $this->openPageAlias(
            'action-list-getpostallowed',
            '&tx_czsimplecal_pi1[startDate]=2010-07-01&tx_czsimplecal_pi1[endDate]=2010-07-08'
        );
        ...
        $this->assertEquals(
            2,
            $this->selenium->getXpathCount('//*[contains(@class, "vevent")]'),
            'endDate can be overridden if enabled'
        );

        // Override orderBy
        $this->openPageAlias(
            'action-list-getpostallowed',
            '&tx_czsimplecal_pi1[orderBy]=endDate&tx_czsimplecal_pi1[startDate]=2010-01-01 00:00:00'
            . '&tx_czsimplecal_pi1[endDate]=2010-01-07 23:59:59'
        );
        $this->assertSame(
            'overlong event',
            $this->selenium->getText('//div[@class="vevent"][last()]//h5'),
            'orderBy can be overridden if enabled'
        );

        // Override order
        $this->openPageAlias(
            'action-list-getpostallowed',
            '&tx_czsimplecal_pi1[order]=desc&tx_czsimplecal_pi1[startDate]=2010-01-01 00:00:00'
            . '&tx_czsimplecal_pi1[endDate]=2010-01-07 23:59:59'
        );
        $this->assertSame(
            'Wed, January 6, 2010',
            $this->selenium->getText('//div[contains(@class, "vcalendar-list")]//h4'),
            'order can be overridden if enabled'
        );

        // Override excludeOverlongEvents
        $this->openPageAlias(
            'action-list-getpostallowed',
            '&tx_czsimplecal_pi1[excludeOverlongEvents]=1&tx_czsimplecal_pi1[startDate]=2010-01-01 00:00:00'
            . '&tx_czsimplecal_pi1[endDate]=2010-01-07 23:59:59'
        );
        $this->assertElementNotPresent(
            '//h5[. = "overlong event"]',
            'excludeOverlongEvents can be overridden if enabled'
        );

        // Override includeStartedEvents
        $this->openPageAlias(
            'action-list-getpostallowed',
            '&tx_czsimplecal_pi1[includeStartedEvents]=1&tx_czsimplecal_pi1[startDate]=2010-01-01 00:00:00'
            . '&tx_czsimplecal_pi1[endDate]=2010-01-07 23:59:59'
        );
        $this->assertElementPresent(
            '//h5[. = "already started event"]',
            'includeStartedEvents can be overridden if enabled'
        );
    }
    ...
    public function testActionMinimonth()
    {
        ...
        foreach ($months as $month) {
            ...
            $this->assertContains(
                ...
                $this->selenium->getText('//table//thead//tr[1]'),
                $month . ': header label is correct and in english'
            );
            ...
            $this->assertSame(
                ...
                $headerDays,
                $month . ': a value for each day of the week is shown.'
            );
            ...
            $this->assertEquals(
                $displayedWeeks,
                $this->selenium->getXPathCount('//table//tbody//tr/td[1][contains(@class, "cal-minimonth-woy")]'),
                $month . ': every week has a week count'
            );
            $this->assertEquals(
                7 * $displayedWeeks,
                $this->selenium->getXPathCount('//table//tbody//td[contains(@class, "cal-minimonth-day")]'),
                $month . ': every week is filled with 7 days.'
            );

            // Check setting of off-days and that there is at least one day of the month in each row
            $daysOfMonthInFirstLine = $this->selenium->getXPathCount(
                '//table//tbody/tr[1]/td[contains(@class, "cal-minimonth-day")][. < 8]'
            ); // Select all <td>'s having a lower value than 08
            $this->assertGreaterThan(
                0,
                $daysOfMonthInFirstLine,
                $month . ': at least one day of this month is shown in first row'
            );
            $this->assertEquals(
                7 - $daysOfMonthInFirstLine,
                $this->selenium->getXPathCount('//table//tbody/tr[1]//td[contains(@class, "cal-minimonth-dayoff")]'),
                $month . ': all days not of this month are marked with a class in first week.'
            );

            $daysOfMonthInLastLine = $this->selenium->getXPathCount(
                '//table//tbody/tr[last()]/td[contains(@class, "cal-minimonth-day")][. > 20]'
            ); // Select all <td>'s having a higher value than 20
            $this->assertGreaterThan(
                0,
                $daysOfMonthInLastLine,
                $month . ': at least one day of this month is shown in last row'
            );

            $this->assertEquals(
                7 - $daysOfMonthInLastLine,
                $this->selenium->getXPathCount(
                    '//table//tbody/tr[last()]//td[contains(@class, "cal-minimonth-dayoff")]'
                ),
                $month . ': all days not of this month are marked with a class in last week.'
            );
        }
        ...
        $this->assertEquals(
            '2 events',
            $this->selenium->getAttribute('//td//a[. = "18"]/@title'),
            'multiple events use the plural in title'
        );
        $this->assertElementNotPresent('link=19', 'tuesday is not linked');
        $this->assertEquals(
            'No events',
            $this->selenium->getAttribute('//td//span[. = "19"]/@title'),
            'no events have correct title'
        );
        $this->assertElementPresent('link=20', 'wednesday is linked');
        $this->assertEquals(
            '1 event',
            $this->selenium->getAttribute('//td//a[. = "20"]/@title'),
            'single event uses the singular in title'
        );
        $this->assertElementNotPresent('link=21', 'thursday is not linked');
        $this->assertElementPresent('link=22', 'friday is linked');
        $this->assertEquals(
            '1 event',
            $this->selenium->getAttribute('//td//a[. = "22"]/@title'),
            'correct event count on friday'
        );
        $this->assertElementPresent('link=23', 'saturday is linked');
        $this->assertEquals(
            '1 event',
            $this->selenium->getAttribute('//td//a[. = "23"]/@title'),
            'correct event count on saturday'
        );
    ...
    public function testActionShow()
    {
        ...
        $this->assertElementPresent(
            'css=div.vcalendar-event',
            'show-view is shown when clicking on an event in the list'
        );
    }
    ...
    public function testDefaultTemplate()
    {
        ...
        $this->assertEquals(
            ...
            $this->selenium->getAttribute('//link[@rel="profile"]/@href'),
            'hCalendar profile is loaded in LIST view'
        );
        ...
        $this->assertElementPresent(
            '//div[contains(@class, "vevent")]//*[contains(@class, "summary")]',
        ...
        $this->assertElementPresent(
            '//div[contains(@class, "vevent")]//a[contains(@class, "url")]',
            'an "url" is present'
        );
        ...
        $this->assertElementPresent(
            '//div[contains(@class, "vcalendar-event")]',
            'the opened view shows an event in the SHOW view'
        );

        $this->assertEquals(
            'http://microformats.org/profile/hcalendar',
            $this->selenium->getAttribute('//link[@rel="profile"]/@href'),
            'hCalendar profile is loaded in SHOW view'
        );
        ...
        $this->assertElementPresent(
            '//div[contains(@class, "vevent")]//*[contains(@class, "summary")]',
            'a "summary" is present in SHOW view'
        );
        $this->assertNotEquals(
            '',
            $this->selenium->getText('//div[contains(@class, "vevent")]//*[contains(@class, "summary")]'),
    ...
    protected function openPageAlias($alias, $parameters = null)
    {
        ...
        $this->assertNotContains(
            ...
            sprintf('Page with alias "%s" does exist.', $alias)
        );
    }
}
